...
/**
 * GlobalCatalogueInstallIndicator — floating dock for CWICR catalogue
 * installs. Mirrors `GlobalUploadIndicator` so the user gets a familiar
 * pattern: kick off an install on /match-elements, navigate freely, see a
 * pill in the corner, get a check-mark + toast when it finishes.
 *
 * Renders nothing when no jobs exist. Stacks above the BIM and DWG dock
...
// Stage label fallbacks. The catalogue install POST is a single
// monolithic call — the FE has no real progress signal — so we rotate
// the user-facing label through these phases on a wall-clock heuristic.
...
export function GlobalCatalogueInstallIndicator() {
  ...
  const jobs = useCatalogueInstallStore((s) => s.jobs);
  const dismissJob = useCatalogueInstallStore((s) => s.dismissJob);
  ...
  // Ticker so elapsed-time labels refresh
  useEffect(() => {
  ...
  // Auto-dismiss completed jobs: 6 s on success, 30 s on error so the
  // user has time to read the failure reason but the dock doesn't pile
  // up. The component re-runs when `finished` changes.
  useEffect(() => {
    ...
    for (const job of finished) {
      ...
      if (remaining <= 0) {
        ...
        timers.push(setTimeout(() => dismissJob(job.region), remaining));
      }
    }
      ...
      <div className="max-h-64 overflow-y-auto divide-y divide-border-light">
        {allJobs.map((job) => (
          <JobRow
            key={job.region}
            job={job}
            onDismiss={() => dismissJob(job.region)}
          />
        ))}
      </div>
    </div>
        ...
        {isActive && (
          <div className="mt-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-emerald-600 font-semibold tabular-nums w-8 text-right">
                {job.progress}%
              </span>
            </div>
