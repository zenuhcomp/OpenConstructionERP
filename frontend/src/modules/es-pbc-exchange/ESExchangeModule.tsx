...
import { ES_TEMPLATE, ES_TRADE_SECTIONS } from './esTemplate';
...
type ESExportFormat = 'pbc-detailed' | 'pbc-summary';
    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t('es.preview', { defaultValue: 'Preview' })}: {positions.length}{' '}
          {t('es.positions', { defaultValue: 'positions' })}
        </span>
        {positions.length > 20 && (
          <button
          ...
          >
            {showAll
              ? t('es.show_less', { defaultValue: 'Show less' })
              : t('es.show_all', { defaultValue: `Show all ${positions.length}` })}
          </button>
        )}
      </div>
          ...
          <thead>
            <tr className="bg-surface-secondary/50 sticky top-0">
              ...
              <th className="px-3 py-1.5 text-left font-medium text-content-secondary w-32">
                {t('es.classification', { defaultValue: 'PBC Chapter' })}
              </th>
            </tr>
          </thead>
...
export default function ESExchangeModule() {
  ...
  const [exportFormat, setExportFormat] = useState<ESExportFormat>('pbc-detailed');
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(file, ES_TEMPLATE.defaultColumns);
        ...
        if (result.errors.length > 0) {
          ...
          setParseError(
            t('es.parse_error', {
              defaultValue:
                'No positions found in the file. Ensure the file is a valid PBC spreadsheet (CSV, TSV, or XLSX).',
            }),
          );
        } else {
          ...
          addToast({
            type: 'success',
            title: t('es.parsed_ok', { defaultValue: 'File parsed successfully' }),
            message: `${result.positions.length} positions found`,
          });
        }
      } catch {
        setParseError(
          t('es.parse_error_generic', {
            defaultValue: 'Failed to parse the PBC file.',
          }),
        );
      }
    },
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        { method: 'POST', body: JSON.stringify({ positions, source: 'es_pbc_import' }) } as never,
      );
      ...
      addToast({
        ...
        title: t('es.import_complete', { defaultValue: 'Spanish PBC import complete' }),
        message: `${result.imported} positions imported`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('es.import_failed', { defaultValue: 'PBC import failed' }),
  ...
  const includePrices = exportFormat === 'pbc-detailed';

  const handleExport = useCallback(() => {
    if (exportablePositions.length === 0) {
      addToast({
        type: 'warning',
        title: t('es.no_positions', { defaultValue: 'No positions to export' }),
      });
      return;
    }
    setIsExporting(true);
    try {
      ...
      const filename = `${projectName}_${boqName}_ES_${exportFormat === 'pbc-detailed' ? 'PBC_Detailed' : 'PBC_Summary'}.csv`;

      const result = exportToCSV(exportablePositions, ES_TEMPLATE, filename, {
        includePrices,
      });
      ...
      addToast({
        type: 'success',
        title: t('es.export_complete', { defaultValue: 'Spanish PBC export complete' }),
        message: `${result.positionCount} positions exported to ${result.filename}`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('es.export_failed', { defaultValue: 'PBC export failed' }),
        message: err instanceof Error ? err.message : 'Unknown error',
      });
    } finally {
  ...
  const handlePrint = useCallback(() => {
    ...
    printBOQReport(exportablePositions, ES_TEMPLATE, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t('es.title', { defaultValue: 'Spain PBC / Base de Precios Import / Export' })}
          </h1>
      ...
      <div className="flex gap-1 border-b border-border">
        ...
        >
          <Upload size={15} />
          {t('es.tab_import', { defaultValue: 'Import' })}
        </button>
        <button
        ...
        >
          <Download size={15} />
          {t('es.tab_export', { defaultValue: 'Export' })}
        </button>
      </div>
      ...
      {activeTab === 'import' && (
        <div className="space-y-5">
            ...
            {importFile ? (
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t('es.positions_found', { defaultValue: 'positions found' })}
                    {parsedPositions.some((p) => p.unitRate > 0) && (
                      <Badge variant="blue" className="ml-2">
                        {t('es.pbc_detailed', { defaultValue: 'PBC Detailed' })}
                      </Badge>
                    )}
                    {parsedPositions.every((p) => p.unitRate === 0) && (
                      <Badge variant="neutral" className="ml-2">
                        {t('es.pbc_summary', { defaultValue: 'PBC Summary' })}
                      </Badge>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t('es.drop_file', {
                    defaultValue: 'Drop a Spanish PBC file here (Excel or CSV), or',
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                >
                  {t('es.browse', { defaultValue: 'Browse files' })}
                </Button>
                <p className="text-2xs text-content-quaternary">
                  {t('es.formats_hint', {
                    defaultValue: 'Supported: .csv, .tsv, .xlsx (PBC-formatted BOQ)',
                  })}
                </p>
              </div>
            )}
          ...
          {!importFile && <SampleTemplateButton template={ES_TEMPLATE} />}

          {/* PBC Chapters reference */}
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t('es.trades_ref', { defaultValue: 'PBC Chapters Reference' })}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {ES_TRADE_SECTIONS.map((sec) => (
                  <span
                    key={sec.code}
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-5">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t('es.target_boq', { defaultValue: 'Import Target' })}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      — {t('es.select_boq', { defaultValue: 'Select BOQ' })} —
                    </option>
                    {importBoqs.map((b) => (
                ...
                <div className="flex items-end">
                  ...
                  >
                    {isImporting
                      ? t('es.importing', { defaultValue: 'Importing...' })
                      : t('es.import_btn', {
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="text-content-primary">
                  ...
                  {t('es.positions_imported', { defaultValue: 'positions imported' })}
                </span>
              </div>
              {importResult.errors.length > 0 && (
              ...
              {importResult.errors.length === 0 && (
                ...
                >
                  {t('es.open_boq', { defaultValue: 'Open in BOQ editor to review & validate \u2192' })}
                </Link>
              )}
            </div>
          )}
      ...
      {activeTab === 'export' && (
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t('es.source_boq', { defaultValue: '1. Select BOQ to Export' })}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
              ...
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t('es.export_format', { defaultValue: 'Format' })}
                </label>
                <select
                  value={exportFormat}
                  onChange={(e) => setExportFormat(e.target.value as ESExportFormat)}
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
                  <option value="pbc-detailed">
                    {t('es.pbc_detailed', { defaultValue: 'PBC Detailed' })} —{' '}
                    {t('es.with_prices', { defaultValue: 'with prices' })}
                  </option>
                  <option value="pbc-summary">
                    {t('es.pbc_summary', { defaultValue: 'PBC Summary' })} —{' '}
                    {t('es.no_prices', { defaultValue: 'quantities only' })}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-content-primary">
                  {t('es.export_summary', { defaultValue: '2. Export Summary' })}
                </h3>
                <button
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t('es.hide_preview', { defaultValue: 'Hide preview' })
                    : t('es.show_preview', { defaultValue: 'Show preview' })}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('es.sections', { defaultValue: 'Sections' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportablePositions.filter((p) => p.isSection).length}
                  </div>
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('es.format_label', { defaultValue: 'Format' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportFormat === 'pbc-detailed' ? 'Detailed' : 'Summary'}
                  </div>
                </div>
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('es.prices_label', { defaultValue: 'Prices' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {includePrices
              ...
              <div className="flex gap-3">
                ...
                >
                  {t('es.export_btn', {
                    defaultValue: `Export as ${exportFormat === 'pbc-detailed' ? 'PBC Detailed' : 'PBC Summary'} CSV`,
                  })}
                </Button>
                <Button variant="secondary" icon={<Printer size={15} />} onClick={handlePrint}>
                  {t('es.print_btn', { defaultValue: 'Print / PDF' })}
                </Button>
              </div>
            </div>
          )}
          ...
          {exportBoqId && exportablePositions.length === 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-8 text-center">
              ...
              <p className="text-sm text-content-tertiary">
                {t('es.no_positions', { defaultValue: 'This BOQ has no positions to export.' })}
              </p>
            </div>
          )}
        </div>
