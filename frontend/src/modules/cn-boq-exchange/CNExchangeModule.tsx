...
import { CN_TEMPLATE, CN_TRADE_SECTIONS } from './cnTemplate';
...
type CNExportFormat = 'gbt-detailed' | 'gbt-summary';
    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t('cn.preview', { defaultValue: 'Preview' })}: {positions.length}{' '}
          {t('cn.positions', { defaultValue: 'positions' })}
        </span>
        {positions.length > 20 && (
          <button
          ...
          >
            {showAll
              ? t('cn.show_less', { defaultValue: 'Show less' })
              : t('cn.show_all', { defaultValue: `Show all ${positions.length}` })}
          </button>
        )}
      </div>
          ...
          <thead>
            <tr className="bg-surface-secondary/50 sticky top-0">
              ...
              <th className="px-3 py-1.5 text-left font-medium text-content-secondary w-32">
                {t('cn.classification', { defaultValue: 'GB/T 50500 Section' })}
              </th>
            </tr>
          </thead>
...
export default function CNExchangeModule() {
  ...
  const [exportFormat, setExportFormat] = useState<CNExportFormat>('gbt-detailed');
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(file, CN_TEMPLATE.defaultColumns);
        ...
        if (result.errors.length > 0) {
          ...
          setParseError(
            t('cn.parse_error', {
              defaultValue:
                'No positions found in the file. Ensure the file is a valid GB/T 50500-formatted BOQ (CSV, TSV, or XLSX).',
            }),
          );
        } else {
          ...
          addToast({
            type: 'success',
            title: t('cn.parsed_ok', { defaultValue: 'File parsed successfully' }),
            message: `${result.positions.length} positions found`,
          });
        }
      } catch {
        setParseError(
          t('cn.parse_error_generic', {
            defaultValue: 'Failed to parse the Chinese BOQ file.',
          }),
        );
      }
    },
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        { method: 'POST', body: JSON.stringify({ positions, source: 'cn_gbt50500_import' }) } as never,
      );
      ...
      addToast({
        ...
        title: t('cn.import_complete', { defaultValue: 'Chinese BOQ import complete' }),
        message: `${result.imported} positions imported`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('cn.import_failed', { defaultValue: 'Chinese import failed' }),
  ...
  const includePrices = exportFormat === 'gbt-detailed';

  const handleExport = useCallback(() => {
    if (exportablePositions.length === 0) {
      addToast({
        type: 'warning',
        title: t('cn.no_positions', { defaultValue: 'No positions to export' }),
      });
      return;
    }
    setIsExporting(true);
    try {
      ...
      const filename = `${projectName}_${boqName}_CN_${exportFormat === 'gbt-detailed' ? 'GBT_Detailed' : 'GBT_Summary'}.csv`;

      const result = exportToCSV(exportablePositions, CN_TEMPLATE, filename, {
        includePrices,
      });
      ...
      addToast({
        type: 'success',
        title: t('cn.export_complete', { defaultValue: 'Chinese BOQ export complete' }),
        message: `${result.positionCount} positions exported to ${result.filename}`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('cn.export_failed', { defaultValue: 'Chinese export failed' }),
        message: err instanceof Error ? err.message : 'Unknown error',
      });
    } finally {
  ...
  const handlePrint = useCallback(() => {
    ...
    printBOQReport(exportablePositions, CN_TEMPLATE, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t('cn.title', { defaultValue: 'China BOQ Import / Export' })}
          </h1>
          <p className="text-sm text-content-tertiary">
            {t('cn.subtitle', {
              defaultValue: 'Exchange Bills of Quantities in GB/T 50500 format (Excel / CSV)',
            })}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-border">
        ...
        >
          <Upload size={15} />
          {t('cn.tab_import', { defaultValue: 'Import' })}
        </button>
        <button
        ...
        >
          <Download size={15} />
          {t('cn.tab_export', { defaultValue: 'Export' })}
        </button>
      </div>
      ...
      {activeTab === 'import' && (
        <div className="space-y-5">
            ...
            {importFile ? (
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t('cn.positions_found', { defaultValue: 'positions found' })}
                    {parsedPositions.some((p) => p.unitRate > 0) && (
                      <Badge variant="blue" className="ml-2">
                        {t('cn.detailed', { defaultValue: 'GB/T Detailed' })}
                      </Badge>
                    )}
                    {parsedPositions.every((p) => p.unitRate === 0) && (
                      <Badge variant="neutral" className="ml-2">
                        {t('cn.summary', { defaultValue: 'GB/T Summary' })}
                      </Badge>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t('cn.drop_file', {
                    defaultValue: 'Drop a Chinese BOQ file here (Excel or CSV), or',
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                >
                  {t('cn.browse', { defaultValue: 'Browse files' })}
                </Button>
                <p className="text-2xs text-content-quaternary">
                  {t('cn.formats_hint', {
                    defaultValue: 'Supported: .csv, .tsv, .xlsx (GB/T 50500-formatted BOQ)',
                  })}
                </p>
              </div>
            )}
          ...
          {!importFile && <SampleTemplateButton template={CN_TEMPLATE} />}

          {/* GB/T 50500 Trade Sections reference */}
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t('cn.trades_ref', { defaultValue: 'GB/T 50500 Trade Sections Reference' })}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {CN_TRADE_SECTIONS.map((sec) => (
                  <span
                    key={sec.code}
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-5">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t('cn.target_boq', { defaultValue: 'Import Target' })}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      — {t('cn.select_boq', { defaultValue: 'Select BOQ' })} —
                    </option>
                    {importBoqs.map((b) => (
                ...
                <div className="flex items-end">
                  ...
                  >
                    {isImporting
                      ? t('cn.importing', { defaultValue: 'Importing...' })
                      : t('cn.import_btn', {
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="text-content-primary">
                  ...
                  {t('cn.positions_imported', { defaultValue: 'positions imported' })}
                </span>
              </div>
              {importResult.errors.length > 0 && (
              ...
              {importResult.errors.length === 0 && (
                ...
                >
                  {t('cn.open_boq', { defaultValue: 'Open in BOQ editor to review & validate \u2192' })}
                </Link>
              )}
            </div>
          )}
      ...
      {activeTab === 'export' && (
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t('cn.source_boq', { defaultValue: '1. Select BOQ to Export' })}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
              ...
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t('cn.export_format', { defaultValue: 'Format' })}
                </label>
                <select
                  value={exportFormat}
                  onChange={(e) => setExportFormat(e.target.value as CNExportFormat)}
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
                  <option value="gbt-detailed">
                    {t('cn.detailed', { defaultValue: 'GB/T Detailed' })} —{' '}
                    {t('cn.with_prices', { defaultValue: 'with prices' })}
                  </option>
                  <option value="gbt-summary">
                    {t('cn.summary', { defaultValue: 'GB/T Summary' })} —{' '}
                    {t('cn.no_prices', { defaultValue: 'quantities only' })}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-content-primary">
                  {t('cn.export_summary', { defaultValue: '2. Export Summary' })}
                </h3>
                <button
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t('cn.hide_preview', { defaultValue: 'Hide preview' })
                    : t('cn.show_preview', { defaultValue: 'Show preview' })}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('cn.sections', { defaultValue: 'Sections' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportablePositions.filter((p) => p.isSection).length}
                  </div>
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('cn.format_label', { defaultValue: 'Format' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportFormat === 'gbt-detailed' ? 'Detailed' : 'Summary'}
                  </div>
                </div>
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('cn.prices_label', { defaultValue: 'Prices' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {includePrices
              ...
              <div className="flex gap-3">
                ...
                >
                  {t('cn.export_btn', {
                    defaultValue: `Export as ${exportFormat === 'gbt-detailed' ? 'GB/T Detailed' : 'GB/T Summary'} CSV`,
                  })}
                </Button>
                <Button variant="secondary" icon={<Printer size={15} />} onClick={handlePrint}>
                  {t('cn.print_btn', { defaultValue: 'Print / PDF' })}
                </Button>
              </div>
            </div>
          )}
          ...
          {exportBoqId && exportablePositions.length === 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-8 text-center">
              ...
              <p className="text-sm text-content-tertiary">
                {t('cn.no_positions', { defaultValue: 'This BOQ has no positions to export.' })}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="flex items-start gap-2 text-xs text-content-quaternary">
        ...
        <p>
          {t('cn.info', {
            defaultValue:
              'GB/T 50500 (\u5DE5\u7A0B\u91CF\u6E05\u5355\u8BA1\u4EF7\u89C4\u8303) is the national standard for construction BOQ pricing in China. Compatible with CEC (China Engineering Cost) guidelines, provincial quota systems, and standard Chinese construction estimation workflows.',
          })}
        </p>
      </div>
    </div>
