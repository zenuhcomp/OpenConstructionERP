...
import { BR_TEMPLATE, BR_TRADE_SECTIONS } from './brTemplate';
...
type BRExportFormat = 'sinapi-detailed' | 'sinapi-summary';
    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t('br.preview', { defaultValue: 'Preview' })}: {positions.length}{' '}
          {t('br.positions', { defaultValue: 'positions' })}
        </span>
        {positions.length > 20 && (
          <button
          ...
          >
            {showAll
              ? t('br.show_less', { defaultValue: 'Show less' })
              : t('br.show_all', { defaultValue: `Show all ${positions.length}` })}
          </button>
        )}
      </div>
          ...
          <thead>
            <tr className="bg-surface-secondary/50 sticky top-0">
              ...
              <th className="px-3 py-1.5 text-left font-medium text-content-secondary w-32">
                {t('br.classification', { defaultValue: 'SINAPI Section' })}
              </th>
            </tr>
          </thead>
...
export default function BRExchangeModule() {
  ...
  const [exportFormat, setExportFormat] = useState<BRExportFormat>('sinapi-detailed');
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(file, BR_TEMPLATE.defaultColumns);
        ...
        if (result.errors.length > 0) {
          ...
          setParseError(
            t('br.parse_error', {
              defaultValue:
                'No positions found in the file. Ensure the file is a valid SINAPI/TCPO-formatted BOQ (CSV, TSV, or XLSX).',
            }),
          );
        } else {
          ...
          addToast({
            type: 'success',
            title: t('br.parsed_ok', { defaultValue: 'File parsed successfully' }),
            message: `${result.positions.length} positions found`,
          });
        }
      } catch {
        setParseError(
          t('br.parse_error_generic', {
            defaultValue: 'Failed to parse the SINAPI/TCPO file.',
          }),
        );
      }
    },
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        { method: 'POST', body: JSON.stringify({ positions, source: 'br_sinapi_import' }) } as never,
      );
      ...
      addToast({
        ...
        title: t('br.import_complete', { defaultValue: 'Brazilian SINAPI import complete' }),
        message: `${result.imported} positions imported`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('br.import_failed', { defaultValue: 'SINAPI import failed' }),
  ...
  const includePrices = exportFormat === 'sinapi-detailed';

  const handleExport = useCallback(() => {
    if (exportablePositions.length === 0) {
      addToast({
        type: 'warning',
        title: t('br.no_positions', { defaultValue: 'No positions to export' }),
      });
      return;
    }
    setIsExporting(true);
    try {
      ...
      const filename = `${projectName}_${boqName}_BR_${exportFormat === 'sinapi-detailed' ? 'SINAPI_Detailed' : 'SINAPI_Summary'}.csv`;

      const result = exportToCSV(exportablePositions, BR_TEMPLATE, filename, {
        includePrices,
      });
      ...
      addToast({
        type: 'success',
        title: t('br.export_complete', { defaultValue: 'Brazilian SINAPI export complete' }),
        message: `${result.positionCount} positions exported to ${result.filename}`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('br.export_failed', { defaultValue: 'SINAPI export failed' }),
        message: err instanceof Error ? err.message : 'Unknown error',
      });
    } finally {
  ...
  const handlePrint = useCallback(() => {
    ...
    printBOQReport(exportablePositions, BR_TEMPLATE, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t('br.title', { defaultValue: 'Brazil SINAPI / TCPO Import / Export' })}
          </h1>
          <p className="text-sm text-content-tertiary">
            {t('br.subtitle', {
              defaultValue: 'Exchange Bills of Quantities in SINAPI / TCPO format (Excel / CSV)',
            })}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-border">
        ...
        >
          <Upload size={15} />
          {t('br.tab_import', { defaultValue: 'Import' })}
        </button>
        <button
        ...
        >
          <Download size={15} />
          {t('br.tab_export', { defaultValue: 'Export' })}
        </button>
      </div>
      ...
      {activeTab === 'import' && (
        <div className="space-y-5">
            ...
            {importFile ? (
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t('br.positions_found', { defaultValue: 'positions found' })}
                    {parsedPositions.some((p) => p.unitRate > 0) && (
                      <Badge variant="blue" className="ml-2">
                        {t('br.sinapi_detailed', { defaultValue: 'SINAPI Detailed' })}
                      </Badge>
                    )}
                    {parsedPositions.every((p) => p.unitRate === 0) && (
                      <Badge variant="neutral" className="ml-2">
                        {t('br.sinapi_summary', { defaultValue: 'SINAPI Summary' })}
                      </Badge>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t('br.drop_file', {
                    defaultValue: 'Drop a SINAPI/TCPO BOQ file here (Excel or CSV), or',
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                >
                  {t('br.browse', { defaultValue: 'Browse files' })}
                </Button>
                <p className="text-2xs text-content-quaternary">
                  {t('br.formats_hint', {
                    defaultValue: 'Supported: .csv, .tsv, .xlsx (SINAPI/TCPO-formatted BOQ)',
                  })}
                </p>
              </div>
            )}
          ...
          {!importFile && <SampleTemplateButton template={BR_TEMPLATE} />}

          {/* SINAPI Trade Sections reference */}
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t('br.trades_ref', { defaultValue: 'SINAPI Trade Sections Reference' })}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {BR_TRADE_SECTIONS.map((sec) => (
                  <span
                    key={sec.code}
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-5">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t('br.target_boq', { defaultValue: 'Import Target' })}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      — {t('br.select_boq', { defaultValue: 'Select BOQ' })} —
                    </option>
                    {importBoqs.map((b) => (
                ...
                <div className="flex items-end">
                  ...
                  >
                    {isImporting
                      ? t('br.importing', { defaultValue: 'Importing...' })
                      : t('br.import_btn', {
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="text-content-primary">
                  ...
                  {t('br.positions_imported', { defaultValue: 'positions imported' })}
                </span>
              </div>
              {importResult.errors.length > 0 && (
              ...
              {importResult.errors.length === 0 && (
                ...
                >
                  {t('br.open_boq', { defaultValue: 'Open in BOQ editor to review & validate \u2192' })}
                </Link>
              )}
            </div>
          )}
      ...
      {activeTab === 'export' && (
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t('br.source_boq', { defaultValue: '1. Select BOQ to Export' })}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
              ...
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t('br.export_format', { defaultValue: 'Format' })}
                </label>
                <select
                  value={exportFormat}
                  onChange={(e) => setExportFormat(e.target.value as BRExportFormat)}
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
                  <option value="sinapi-detailed">
                    {t('br.sinapi_detailed', { defaultValue: 'SINAPI Detailed' })} —{' '}
                    {t('br.with_prices', { defaultValue: 'with prices' })}
                  </option>
                  <option value="sinapi-summary">
                    {t('br.sinapi_summary', { defaultValue: 'SINAPI Summary' })} —{' '}
                    {t('br.no_prices', { defaultValue: 'quantities only' })}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-content-primary">
                  {t('br.export_summary', { defaultValue: '2. Export Summary' })}
                </h3>
                <button
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t('br.hide_preview', { defaultValue: 'Hide preview' })
                    : t('br.show_preview', { defaultValue: 'Show preview' })}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('br.sections', { defaultValue: 'Sections' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportablePositions.filter((p) => p.isSection).length}
                  </div>
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('br.format_label', { defaultValue: 'Format' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportFormat === 'sinapi-detailed' ? 'Detailed' : 'Summary'}
                  </div>
                </div>
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('br.prices_label', { defaultValue: 'Prices' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {includePrices
              ...
              <div className="flex gap-3">
                ...
                >
                  {t('br.export_btn', {
                    defaultValue: `Export as ${exportFormat === 'sinapi-detailed' ? 'SINAPI Detailed' : 'SINAPI Summary'} CSV`,
                  })}
                </Button>
                <Button variant="secondary" icon={<Printer size={15} />} onClick={handlePrint}>
                  {t('br.print_btn', { defaultValue: 'Print / PDF' })}
                </Button>
              </div>
            </div>
          )}
          ...
          {exportBoqId && exportablePositions.length === 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-8 text-center">
              <Globe size={32} className="mx-auto text-content-quaternary mb-2" />
              <p className="text-sm text-content-tertiary">
                {t('br.no_positions', { defaultValue: 'This BOQ has no positions to export.' })}
              </p>
            </div>
          )}
        </div>
