...
import { CA_TEMPLATE, CA_TRADE_SECTIONS } from './caTemplate';
...
type CAExportFormat = 'masterformat-detailed' | 'ciqs-summary';
    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t('ca.preview', { defaultValue: 'Preview' })}: {positions.length}{' '}
          {t('ca.positions', { defaultValue: 'positions' })}
        </span>
        {positions.length > 20 && (
          <button
          ...
          >
            {showAll
              ? t('ca.show_less', { defaultValue: 'Show less' })
              : t('ca.show_all', { defaultValue: `Show all ${positions.length}` })}
          </button>
        )}
      </div>
          ...
          <thead>
            <tr className="bg-surface-secondary/50 sticky top-0">
              ...
              <th className="px-3 py-1.5 text-left font-medium text-content-secondary w-32">
                {t('ca.classification', { defaultValue: 'MasterFormat/CIQS Code' })}
              </th>
            </tr>
          </thead>
...
export default function CAExchangeModule() {
  ...
  const [exportFormat, setExportFormat] = useState<CAExportFormat>('masterformat-detailed');
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(file, CA_TEMPLATE.defaultColumns);
        ...
        if (result.errors.length > 0) {
          ...
          setParseError(
            t('ca.parse_error', {
              defaultValue:
                'No positions found in the file. Ensure the file is a valid Canadian MasterFormat/CIQS-formatted BOQ (CSV, TSV, or XLSX).',
            }),
          );
        } else {
          ...
          addToast({
            type: 'success',
            title: t('ca.parsed_ok', { defaultValue: 'File parsed successfully' }),
            message: `${result.positions.length} positions found`,
          });
        }
      } catch {
        setParseError(
          t('ca.parse_error_generic', {
            defaultValue: 'Failed to parse the Canadian BOQ file.',
          }),
        );
      }
    },
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        { method: 'POST', body: JSON.stringify({ positions, source: 'ca_import' }) } as never,
      );
      ...
      addToast({
        ...
        title: t('ca.import_complete', { defaultValue: 'Canadian BOQ import complete' }),
        message: `${result.imported} positions imported`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('ca.import_failed', { defaultValue: 'Canadian import failed' }),
  ...
  const handleExport = useCallback(() => {
    if (exportablePositions.length === 0) {
      addToast({
        type: 'warning',
        title: t('ca.no_positions', { defaultValue: 'No positions to export' }),
      });
      return;
    }
    setIsExporting(true);
    try {
      ...
      const filename = `${projectName}_${boqName}_CA_${exportFormat === 'masterformat-detailed' ? 'MasterFormat' : 'CIQS'}.csv`;

      const result = exportToCSV(exportablePositions, CA_TEMPLATE, filename, {
        includePrices,
      });
      ...
      addToast({
        type: 'success',
        title: t('ca.export_complete', { defaultValue: 'Canadian BOQ export complete' }),
        message: `${result.positionCount} positions exported to ${result.filename}`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('ca.export_failed', { defaultValue: 'Canadian export failed' }),
        message: err instanceof Error ? err.message : 'Unknown error',
      });
    } finally {
  ...
  const handlePrint = useCallback(() => {
    ...
    printBOQReport(exportablePositions, CA_TEMPLATE, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t('ca.title', { defaultValue: 'Canada BOQ Import / Export' })}
          </h1>
          <p className="text-sm text-content-tertiary">
            {t('ca.subtitle', {
              defaultValue:
                'Exchange Bills of Quantities in Canadian MasterFormat / CIQS format (Excel / CSV)',
            })}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-border">
        ...
        >
          <Upload size={15} />
          {t('ca.tab_import', { defaultValue: 'Import' })}
        </button>
        <button
        ...
        >
          <Download size={15} />
          {t('ca.tab_export', { defaultValue: 'Export' })}
        </button>
      </div>
      ...
      {activeTab === 'import' && (
        <div className="space-y-5">
            ...
            {importFile ? (
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t('ca.positions_found', { defaultValue: 'positions found' })}
                    {parsedPositions.some((p) => p.unitRate > 0) && (
                      <Badge variant="blue" className="ml-2">
                        {t('ca.detailed', { defaultValue: 'MasterFormat Detailed' })}
                      </Badge>
                    )}
                    {parsedPositions.every((p) => p.unitRate === 0) && (
                      <Badge variant="neutral" className="ml-2">
                        {t('ca.summary', { defaultValue: 'CIQS Summary' })}
                      </Badge>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t('ca.drop_file', {
                    defaultValue: 'Drop a Canadian BOQ file here (Excel or CSV), or',
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                >
                  {t('ca.browse', { defaultValue: 'Browse files' })}
                </Button>
                <p className="text-2xs text-content-quaternary">
                  {t('ca.formats_hint', {
                    defaultValue: 'Supported: .csv, .tsv, .xlsx (MasterFormat/CIQS-formatted BOQ)',
                  })}
                </p>
              </div>
            )}
          ...
          {!importFile && <SampleTemplateButton template={CA_TEMPLATE} />}

          {/* Canadian trade sections reference */}
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t('ca.sections_ref', {
                  defaultValue: 'Canadian MasterFormat Trade Sections Reference',
                })}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {CA_TRADE_SECTIONS.map((sec) => (
                  <span
                    key={sec.code}
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-5">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t('ca.target_boq', { defaultValue: 'Import Target' })}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      — {t('ca.select_boq', { defaultValue: 'Select BOQ' })} —
                    </option>
                    {importBoqs.map((b) => (
                ...
                <div className="flex items-end">
                  ...
                  >
                    {isImporting
                      ? t('ca.importing', { defaultValue: 'Importing...' })
                      : t('ca.import_btn', {
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="text-content-primary">
                  ...
                  {t('ca.positions_imported', { defaultValue: 'positions imported' })}
                </span>
              </div>
              {importResult.errors.length > 0 && (
              ...
              {importResult.errors.length === 0 && (
                ...
                >
                  {t('ca.open_boq', { defaultValue: 'Open in BOQ editor to review & validate \u2192' })}
                </Link>
              )}
            </div>
          )}
      ...
      {activeTab === 'export' && (
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t('ca.source_boq', { defaultValue: '1. Select BOQ to Export' })}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
              ...
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t('ca.export_format', { defaultValue: 'Format' })}
                </label>
                <select
                  value={exportFormat}
                  onChange={(e) => setExportFormat(e.target.value as CAExportFormat)}
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
                  <option value="masterformat-detailed">
                    {t('ca.detailed', { defaultValue: 'MasterFormat Detailed' })} —{' '}
                    {t('ca.with_prices', { defaultValue: 'with prices' })}
                  </option>
                  <option value="ciqs-summary">
                    {t('ca.summary', { defaultValue: 'CIQS Summary' })} —{' '}
                    {t('ca.no_prices', { defaultValue: 'quantities only' })}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-content-primary">
                  {t('ca.export_summary', { defaultValue: '2. Export Summary' })}
                </h3>
                <button
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t('ca.hide_preview', { defaultValue: 'Hide preview' })
                    : t('ca.show_preview', { defaultValue: 'Show preview' })}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('ca.sections', { defaultValue: 'Sections' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportablePositions.filter((p) => p.isSection).length}
                  </div>
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('ca.format_label', { defaultValue: 'Format' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportFormat === 'masterformat-detailed' ? 'MF' : 'CIQS'}
                  </div>
                </div>
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('ca.prices_label', { defaultValue: 'Prices' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {includePrices
              ...
              <div className="flex gap-3">
                ...
                >
                  {t('ca.export_btn', {
                    defaultValue: `Export as ${exportFormat === 'masterformat-detailed' ? 'MasterFormat' : 'CIQS'} CSV`,
                  })}
                </Button>
                <Button variant="secondary" icon={<Printer size={15} />} onClick={handlePrint}>
                  {t('ca.print_btn', { defaultValue: 'Print / PDF' })}
                </Button>
              </div>
            </div>
          )}
          ...
          {exportBoqId && exportablePositions.length === 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-8 text-center">
              <MapPin size={32} className="mx-auto text-content-quaternary mb-2" />
              <p className="text-sm text-content-tertiary">
                {t('ca.no_positions', { defaultValue: 'This BOQ has no positions to export.' })}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="flex items-start gap-2 text-xs text-content-quaternary">
        ...
        <p>
          {t('ca.info', {
            defaultValue:
              'Canadian construction uses CSI MasterFormat (adapted for Canada by CSC) and CIQS (Canadian Institute of Quantity Surveyors) standards. Compatible with MERX, Altus Group, and provincial tendering systems.',
          })}
        </p>
      </div>
    </div>
