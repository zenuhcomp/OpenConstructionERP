...
import { PL_TEMPLATE, PL_TRADE_SECTIONS } from './plTemplate';
...
type PLExportFormat = 'knr-detailed' | 'knr-summary';
    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t('pl.preview', { defaultValue: 'Preview' })}: {positions.length}{' '}
          {t('pl.positions', { defaultValue: 'positions' })}
        </span>
        {positions.length > 20 && (
          <button
          ...
          >
            {showAll
              ? t('pl.show_less', { defaultValue: 'Show less' })
              : t('pl.show_all', { defaultValue: `Show all ${positions.length}` })}
          </button>
        )}
      </div>
          ...
          <thead>
            <tr className="bg-surface-secondary/50 sticky top-0">
              ...
              <th className="px-3 py-1.5 text-left font-medium text-content-secondary w-32">
                {t('pl.classification', { defaultValue: 'KNR Section' })}
              </th>
            </tr>
          </thead>
...
export default function PLExchangeModule() {
  ...
  const [exportFormat, setExportFormat] = useState<PLExportFormat>('knr-detailed');
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(file, PL_TEMPLATE.defaultColumns);
        ...
        if (result.errors.length > 0) {
          ...
          setParseError(
            t('pl.parse_error', {
              defaultValue:
                'No positions found in the file. Ensure the file is a valid KNR/KNNR-formatted BOQ (CSV, TSV, or XLSX).',
            }),
          );
        } else {
          ...
          addToast({
            type: 'success',
            title: t('pl.parsed_ok', { defaultValue: 'File parsed successfully' }),
            message: `${result.positions.length} positions found`,
          });
        }
      } catch {
        setParseError(
          t('pl.parse_error_generic', {
            defaultValue: 'Failed to parse the Polish KNR BOQ file.',
          }),
        );
      }
    },
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        { method: 'POST', body: JSON.stringify({ positions, source: 'pl_knr_import' }) } as never,
      );
      ...
      addToast({
        ...
        title: t('pl.import_complete', { defaultValue: 'Polish KNR BOQ import complete' }),
        message: `${result.imported} positions imported`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('pl.import_failed', { defaultValue: 'Polish KNR import failed' }),
  ...
  const includePrices = exportFormat === 'knr-detailed';

  const handleExport = useCallback(() => {
    if (exportablePositions.length === 0) {
      addToast({
        type: 'warning',
        title: t('pl.no_positions', { defaultValue: 'No positions to export' }),
      });
      return;
    }
    setIsExporting(true);
    try {
      ...
      const filename = `${projectName}_${boqName}_PL_${exportFormat === 'knr-detailed' ? 'KNR_Detailed' : 'KNR_Summary'}.csv`;

      const result = exportToCSV(exportablePositions, PL_TEMPLATE, filename, {
        includePrices,
      });
      ...
      addToast({
        type: 'success',
        title: t('pl.export_complete', { defaultValue: 'Polish KNR BOQ export complete' }),
        message: `${result.positionCount} positions exported to ${result.filename}`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('pl.export_failed', { defaultValue: 'Polish KNR export failed' }),
        message: err instanceof Error ? err.message : 'Unknown error',
      });
    } finally {
  ...
  const handlePrint = useCallback(() => {
    ...
    printBOQReport(exportablePositions, PL_TEMPLATE, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t('pl.title', { defaultValue: 'Poland KNR Import / Export' })}
          </h1>
          <p className="text-sm text-content-tertiary">
            {t('pl.subtitle', {
              defaultValue: 'Exchange Bills of Quantities in KNR / KNNR format (Excel / CSV)',
            })}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-border">
        ...
        >
          <Upload size={15} />
          {t('pl.tab_import', { defaultValue: 'Import' })}
        </button>
        <button
        ...
        >
          <Download size={15} />
          {t('pl.tab_export', { defaultValue: 'Export' })}
        </button>
      </div>
      ...
      {activeTab === 'import' && (
        <div className="space-y-5">
            ...
            {importFile ? (
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t('pl.positions_found', { defaultValue: 'positions found' })}
                    {parsedPositions.some((p) => p.unitRate > 0) && (
                      <Badge variant="blue" className="ml-2">
                        {t('pl.detailed', { defaultValue: 'KNR Detailed' })}
                      </Badge>
                    )}
                    {parsedPositions.every((p) => p.unitRate === 0) && (
                      <Badge variant="neutral" className="ml-2">
                        {t('pl.summary', { defaultValue: 'KNR Summary' })}
                      </Badge>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t('pl.drop_file', {
                    defaultValue: 'Drop a Polish KNR/KNNR BOQ file here (Excel or CSV), or',
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                >
                  {t('pl.browse', { defaultValue: 'Browse files' })}
                </Button>
                <p className="text-2xs text-content-quaternary">
                  {t('pl.formats_hint', {
                    defaultValue: 'Supported: .csv, .tsv, .xlsx (KNR/KNNR-formatted BOQ)',
                  })}
                </p>
              </div>
            )}
          ...
          {!importFile && <SampleTemplateButton template={PL_TEMPLATE} />}

          {/* KNR Trade Sections reference */}
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t('pl.trades_ref', { defaultValue: 'KNR Trade Sections Reference' })}
              </div>
              <div className="flex flex-wrap gap-1.5">
                {PL_TRADE_SECTIONS.map((sec) => (
                  <span
                    key={sec.code}
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-5">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t('pl.target_boq', { defaultValue: 'Import Target' })}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      — {t('pl.select_boq', { defaultValue: 'Select BOQ' })} —
                    </option>
                    {importBoqs.map((b) => (
                ...
                <div className="flex items-end">
                  ...
                  >
                    {isImporting
                      ? t('pl.importing', { defaultValue: 'Importing...' })
                      : t('pl.import_btn', {
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="text-content-primary">
                  ...
                  {t('pl.positions_imported', { defaultValue: 'positions imported' })}
                </span>
              </div>
              {importResult.errors.length > 0 && (
              ...
              {importResult.errors.length === 0 && (
                ...
                >
                  {t('pl.open_boq', { defaultValue: 'Open in BOQ editor to review & validate \u2192' })}
                </Link>
              )}
            </div>
          )}
      ...
      {activeTab === 'export' && (
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t('pl.source_boq', { defaultValue: '1. Select BOQ to Export' })}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
              ...
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t('pl.export_format', { defaultValue: 'Format' })}
                </label>
                <select
                  value={exportFormat}
                  onChange={(e) => setExportFormat(e.target.value as PLExportFormat)}
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
                  <option value="knr-detailed">
                    {t('pl.detailed', { defaultValue: 'KNR Detailed' })} —{' '}
                    {t('pl.with_prices', { defaultValue: 'with prices' })}
                  </option>
                  <option value="knr-summary">
                    {t('pl.summary', { defaultValue: 'KNR Summary' })} —{' '}
                    {t('pl.no_prices', { defaultValue: 'quantities only' })}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-content-primary">
                  {t('pl.export_summary', { defaultValue: '2. Export Summary' })}
                </h3>
                <button
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t('pl.hide_preview', { defaultValue: 'Hide preview' })
                    : t('pl.show_preview', { defaultValue: 'Show preview' })}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('pl.sections', { defaultValue: 'Sections' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportablePositions.filter((p) => p.isSection).length}
                  </div>
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('pl.format_label', { defaultValue: 'Format' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportFormat === 'knr-detailed' ? 'Detailed' : 'Summary'}
                  </div>
                </div>
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('pl.prices_label', { defaultValue: 'Prices' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {includePrices
              ...
              <div className="flex gap-3">
                ...
                >
                  {t('pl.export_btn', {
                    defaultValue: `Export as ${exportFormat === 'knr-detailed' ? 'KNR Detailed' : 'KNR Summary'} CSV`,
                  })}
                </Button>
                <Button variant="secondary" icon={<Printer size={15} />} onClick={handlePrint}>
                  {t('pl.print_btn', { defaultValue: 'Print / PDF' })}
                </Button>
              </div>
            </div>
          )}
          ...
          {exportBoqId && exportablePositions.length === 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-8 text-center">
              ...
              <p className="text-sm text-content-tertiary">
                {t('pl.no_positions', { defaultValue: 'This BOQ has no positions to export.' })}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="flex items-start gap-2 text-xs text-content-quaternary">
        ...
        <p>
          {t('pl.info', {
            defaultValue:
              'KNR (Katalog Naklad\u00f3w Rzeczowych) and KNNR (Kosztorysowe Normy Naklad\u00f3w Rzeczowych) are the Polish standard cost catalogues for construction estimating. Compatible with Norma Pro, Zuzia, Rodos, and other Polish estimating software. Organized by trade sections following Polish construction classification.',
          })}
        </p>
      </div>
    </div>
